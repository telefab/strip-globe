...
#define GLOBE_BASEADDR 0x43C30000

// Addresses and sizes of the 
//#define SPEED_BASEADDR 0x43C04C90
#define SPEED_OFFSET 0x4C90
#define SPEED_SIZE 0x4
//#define RND_BASEADDR 0x43C04C98
#define RND_OFFSET 0x4C98
#define RND_SIZE 0x4
//#define CNTRL_BASEADDR 0x43C04C94
#define CNTRL_OFFSET 0x4C94
#define CNTRL_SIZE 0x4
//#define CHAR_BASEADDR 0x43C04C9C
#define CHAR_OFFSET 0x4C9C
...
int speed_init(void)
{
  ...
  // protect the memory region 
  request_mem_region(GLOBE_BASEADDR + SPEED_OFFSET, SPEED_SIZE, "speed");

  // remap the BASE_ADDR memory region
  speed_addr = ioremap(GLOBE_BASEADDR + SPEED_OFFSET, SPEED_SIZE);
  if (speed_addr == NULL) {
...
int rnd_init(void)
{
  ...
  // protect the memory region 
  request_mem_region(GLOBE_BASEADDR + RND_OFFSET, RND_SIZE, "rnd");

  // remap the BASE_ADDR memory region
  rnd_addr = ioremap(GLOBE_BASEADDR + RND_OFFSET, RND_SIZE);
  if (rnd_addr == NULL) {
...
int cntrl_init(void)
{
  ...
  // protect the memory region 
  request_mem_region(GLOBE_BASEADDR + CNTRL_OFFSET, CNTRL_SIZE, "cntrl");

  // remap the BASE_ADDR memory region
  cntrl_addr = ioremap(GLOBE_BASEADDR + CNTRL_OFFSET, CNTRL_SIZE);
  if (cntrl_addr == NULL) {
...
void cleanup_custom_module(void)
{
  ...
  iounmap(rnd_addr);
  iounmap(cntrl_addr);
  release_mem_region(GLOBE_BASEADDR + SPEED_OFFSET, SPEED_SIZE);
  release_mem_region(GLOBE_BASEADDR + RND_OFFSET, RND_SIZE);
  release_mem_region(GLOBE_BASEADDR + CNTRL_OFFSET, CNTRL_SIZE);
...
int init_custom_module(void)
{
  ...
  result = speed_init();
  if (result !=0) { 
	  printk(KERN_ERR "Error when initializing speed\n");
	  return result;
  }
  
  result = rnd_init();
  if (result !=0) {
	  printk(KERN_ERR "Error when initializing rnd\n");
	  return result;
  }
  
  result = cntrl_init();
  if (result !=0) {
	  printk(KERN_ERR "Error when initializing cntrl\n");
	  return result;
  }
