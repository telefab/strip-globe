...
/*
This demonstrator switchs between two images, the image of the Olympics and a full cyan image.
At the tenth step, the demonstrator copy the read image in the other frame buffer using the control driver.
*/
...
int main(int args, char *argv[])
{
	...
	// Writing theOlympics image in the frame buffer 1
	for(i = 0;i < 100;i++) 
	{
		for(j = 0;j < 48;j++)
		{
			*(fb + (i*48 + j)) = image[47-j][i];
		}
	}
	
	// WR0 : Writing in the frame buffer 0
	// PS_CONTROL : The cntrl says which frame buffer reads the display part of the globe, here the frame buffer 1
	// RAM_ENABLE : The read frame buffer is the 1
	cntrl[0] = WR0 | PS_CONTROL | RAM_ENABLE;
	
	// Sending the value to the drivers
    write(fc, &cntrl, 1);
	
	// Writing a full cyan image in the frame buffer 0
	for(i = 0;i < 100;i++) 
	{
		for(j = 0;j < 49;j++)
		{
	...
	// Loop until all the steps have been passed
	while(k < 20)
	{
		...
		if(k == 10)
		{
			// Copy the image read in the other frame buffer
			cntrl[0] = COPY;
		}
		else if(k % 2 == 0) // Switch the read frame buffer and the written frame buffer
		{
			cntrl[0] = PS_CONTROL;
		}
		else // Switch the read frame buffer and the written frame buffer
		{
			cntrl[0] = PS_CONTROL | RAM_ENABLE;
		}
		write(fc, &cntrl, 1);
